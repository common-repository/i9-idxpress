<?php
class i9_ListingAreasWidget extends WP_Widget {

	public function __construct() {

		parent::__construct("i9-list-areas", "I-9 IDX Area", array(

			"classname" => "i9-widget-list-areas",
	...
	function widget($args, $instance) {
		...
		foreach ($areaOptions["areas"] as $area) {
			...
			if (preg_match('/^[\w\d\s\-_]+$/', $actualArea)) {
				...
				$encodedArea = urlencode(str_replace(array("-", " "), array("_", "-"), $actualArea));

				$fullAreaUrl = $urlBase . $areaType .'/'. $encodedArea . '/';

			} else if ($areaType == "city") {

				$fullAreaUrl = $urlBase . "?canny-q-Cities=" . urlencode($actualArea);

			} else if ($areaType == "community") {

				$fullAreaUrl = $urlBase . "?canny-q-Communities=" . urlencode($actualArea);

			} else if ($areaType == "tract") {

				$fullAreaUrl = $urlBase . "?canny-q-TractIdentifiers=" . urlencode($actualArea);

			}

			$fullAreaUrl = $fullAreaUrl."page-1";
	...
	function form($instance) {
		...
		wp_enqueue_script('i9idxpress_widget_list_areas', plugins_url('js/widget-list-areas.js', __FILE__ ), array('jquery'), i9_PLUGIN_VERSION, true);
		...
		echo <<<HTML
			...
			<p>
				...
				<select class="widefat" id="{$areaOptionsFieldId}_areaType" name="{$areaOptionsFieldName}[areaType]" onchange="dsWidgetListAreas.SwitchType(this, '{$areaOptionsFieldId}_link_title')">
					...
					<option value="zip" {$selectedAreaType[zip]}>ZipCodes</option>
