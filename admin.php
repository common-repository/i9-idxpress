...
class i9SearchAgent_Admin {
	...
	static function GenerateAdminMenus($icon_url){		
		...
		add_menu_page('IDXMATRIX', 'IDXMATRIX', "manage_options", "i9xpress", "", $icon_url);
	...
	static function Enqueue($hook) {
		...
		if ($hook == 'nav-menus.php' || $hook == 'post.php' || $hook == 'post-new.php') {
			...
			//wp_enqueue_script('i9idxpress_google_maps_geocode_api', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyBwc41AA_F20W7NYuaO7w5uLtItCpJyjsc&v=3.exp&sensor=true&libraries=drawing');
			...
			wp_enqueue_style('i9idxpress_admin_options_style', plugins_url('css/admin-options.css', __FILE__ ), array(), i9_PLUGIN_VERSION);
...
?>
	...
	<div class="wrap metabox-holder">
		...
		<h2>Display Settings</h2>
			...
			<span class="description">This setting allows you to remove links to <a href="https://www.idxmatrix.com">IDXMATRIX</a> that are included in the IDX disclaimer.</span>
			...
			<table class="form-table">
				...
				<tr>
					...
					<th>
						...
						<label for="i9idxpress-RemoveDsDisclaimerLinks">Remove IDXMATRIX links</label>
...
?>
			...
			<p class="error">
				...
				It seems that there was an issue while trying to load the diagnostics from IDXMATIX servers. It's possible that our servers
...
*/
	...
	static function SanitizeOptions($options) {
		...
		if ($full_options = get_option(i9_OPTION_NAME)) {
			...
			// make sure the option to remove idxmatrix links is removed if unchecked
	 ...
	 * We're using the sanitize to capture the POST for these options so we can send them back to the IDXMATRIX API
